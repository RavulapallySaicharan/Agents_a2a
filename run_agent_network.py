...
class AgentNetworkRunner:
    ...
    def _validate_config(self):
        ...
        required_keys = ['agent_type', 'agents', 'connections']
        ...
        if not isinstance(self.config['connections'], list):
            raise ValueError("'connections' must be a list")
            
    def _setup_session(self):
        """Set up the session configuration."""
        self.session_config.create_session(self.session_id)
        self.session_config.update_context(self.session_id, {
    ...
    def _load_agent_modules(self):
        """Load agent modules based on configuration."""
        agent_modules = {}
    ...
    def _initialize_agents(self, agent_modules: Dict[str, Any]):
        """Initialize agent instances."""
        agents = {}
        for agent_config in self.config['agents']:
            try:
                agent_class = agent_modules[agent_config['name']]
                agent = agent_class(**agent_config.get('params', {}))
    ...
    def _setup_connections(self, agents: Dict[str, Any]):
        """Set up connections between agents."""
        for connection in self.config['connections']:
            try:
                source = agents[connection['source']]
                target = agents[connection['target']]
                source.connect(target, connection.get('params', {}))
                logger.info(f"Connected {connection['source']} to {connection['target']}")
            except Exception as e:
                logger.error(f"Failed to set up connection {connection['source']} -> {connection['target']}: {str(e)}")
                raise
                
    def _save_session_state(self, state: str):
    ...
    def run(self, input_data: Any = None):
        ...
        try:
            ...
            agent_modules = self._load_agent_modules()
            agents = self._initialize_agents(agent_modules)
            
            # Set up connections
            self._setup_connections(agents)
            
            # Find the entry point agent
            entry_point = next((agent for agent in agents.values() if agent.is_entry_point()), None)
            if not entry_point:
                raise ValueError("No entry point agent found in the network")
                
            # Run the network
            self._save_session_state("running")
            result = entry_point.process(input_data)
            self._save_session_state("completed")
            ...
            self._save_session_state("failed")
            logger.error(f"Agent network execution failed: {str(e)}")
            raise
            
    def cleanup(self):
        """Clean up the session."""
        try:
            self.session_config.cleanup_all()
            logger.info("Cleaned up all session data")
        except Exception as e:
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python run_agent_network.py <config_file>")
        sys.exit(1)
        
    config_path = sys.argv[1]
    runner = AgentNetworkRunner(config_path)
    
    try:
        result = runner.run()
        print("Agent network execution completed successfully")
